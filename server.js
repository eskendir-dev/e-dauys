...
const db = new sqlite3.Database(DB_PATH, (err) => {
    ...
    else console.log(`БД инициализирована: ${DB_PATH}`);
});

db.serialize(() => {
    db.run(`CREATE TABLE IF NOT EXISTS admins (iin TEXT PRIMARY KEY, email TEXT, otp TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS sessions (token TEXT PRIMARY KEY, iin TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS polls (id TEXT PRIMARY KEY, code TEXT UNIQUE, deadline INTEGER, data_json TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS votes (id INTEGER PRIMARY KEY AUTOINCREMENT, poll_id TEXT, iin TEXT, answers_json TEXT, UNIQUE(poll_id, iin))`);
    db.run(`CREATE TABLE IF NOT EXISTS voter_otp (iin TEXT, poll_code TEXT, otp TEXT, PRIMARY KEY(iin, poll_code))`);
...
app.post('/api/admin/login', (req, res) => {
    ...
    db.get('SELECT * FROM admins WHERE iin = ?', [iin], (err, admin) => {
        ...
        db.run('UPDATE admins SET otp = ? WHERE iin = ?', [otp, iin], () => {
            console.log(`[EMAIL] OTP для админа ${iin}: ${otp}`);
...
app.get('/api/admin/polls', (req, res) => {
    ...
    db.get('SELECT iin FROM sessions WHERE token = ?', [token], (err, session) => {
        ...
        db.all('SELECT id, code, deadline, data_json FROM polls ORDER BY deadline DESC', [], (err, polls) => {
...
app.post('/api/voter/auth', (req, res) => {
    ...
    db.get('SELECT * FROM polls WHERE code = ?', [code], (err, poll) => {
        ...
        if (Date.now() > poll.deadline) return res.status(403).json({ error: 'Опрос завершен' });

        db.get('SELECT id FROM votes WHERE poll_id = ? AND iin = ?', [poll.id, iin], (err, vote) => {
            if (vote) return res.status(403).json({ error: 'Голос уже учтен' });
            const otp = Math.floor(1000 + Math.random() * 9000).toString();
            db.run('INSERT OR REPLACE INTO voter_otp (iin, poll_code, otp) VALUES (?, ?, ?)', [iin, code, otp], () => {
                console.log(`[EMAIL] OTP для избирателя ${iin}: ${otp}`);
                res.json({ message: 'OTP отправлен' });
            });
        });
    });
...
app.post('/api/voter/vote', (req, res) => {
    ...
    db.get('SELECT deadline FROM polls WHERE id = ?', [pollId], (err, poll) => {
        if (!poll || Date.now() > poll.deadline) return res.status(403).json({ error: 'Опрос недоступен' });
        db.run('INSERT INTO votes (poll_id, iin, answers_json) VALUES (?, ?, ?)', [pollId, iin, JSON.stringify(answers)], function(err) {
            if (err) return res.status(403).json({ error: 'Ошибка или голос учтен' });
